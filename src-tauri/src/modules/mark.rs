...
pub fn get_weekly_overview() -> Result<Vec<WeeklyGroup>, String> {
    ...
    // 外层 Key 是周起始日期字符串
    let mut weekly_map: HashMap<String, HashMap<i64, HashMap<i32, (String, Vec<String>)>>> = HashMap::new();

    for mark in marks {
        let (create_time, project_id, project_name, category_id, content) = mark.map_err(|e| e.to_string())?;
        ...
        // 逐级获取或插入 HashMap
        let project_map = weekly_map.entry(week_start.clone()).or_insert_with(HashMap::new);
        let category_map = project_map.entry(project_id).or_insert_with(HashMap::new);
        let (_, contents) = category_map.entry(category_id).or_insert_with(|| (project_name, Vec::new()));
        
        contents.push(content);
    }

    // 3. 将 HashMap 转换为有序的 Vec，并满足你的排序要求
    let mut result: Vec<WeeklyGroup> = weekly_map.iter().map(|(week_start, project_map)| {
        let week_end = get_week_end(week_start); 
        
        let projects: Vec<ProjectGroup> = project_map.iter().map(|(project_id, category_map)| {
            ...
            let mut categories: Vec<CategoryGroup> = category_map.iter().map(|(category_id, (_, contents))| {
                CategoryGroup { category_id: *category_id, contents: contents.clone() }
            }).collect();

            // 分类按照 category_id 升序排序
            categories.sort_by_key(|c| c.category_id);

            ProjectGroup { project_id: *project_id, project_name, categories }
        }).collect();

        WeeklyGroup { week_start: week_start.clone(), week_end, projects }
    }).collect();
